...
$installRootLine = if ($InstallToDesktop) {
  '$installRoot = Join-Path ([Environment]::GetFolderPath("Desktop")) "The Isle Bosch Overlay v2"'
} else {
  '$installRoot = ""'
}
...
`$installerPs1 = Join-Path `$extractRoot "Install-ReptarlandGateway.ps1"
if (-not (Test-Path -LiteralPath `$installerPs1)) {
  throw "Installer script not found in extracted package at '`$installerPs1'."
}

try {
  `$arguments = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", `$installerPs1, "-LaunchAfterInstall")
  if (-not [string]::IsNullOrWhiteSpace(`$installRoot)) {
    `$arguments += @("-InstallRoot", `$installRoot)
  }

  `$process = Start-Process -FilePath "powershell.exe" -ArgumentList `$arguments -WorkingDirectory `$extractRoot -PassThru -Wait
